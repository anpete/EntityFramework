...
FROM [BaseCollectionOnBase] AS [e.BaseCollectionOnBase]
LEFT JOIN (
    SELECT [e.BaseCollectionOnBase.NestedReference].*
    FROM [NestedReferenceBase] AS [e.BaseCollectionOnBase.NestedReference]
    WHERE [e.BaseCollectionOnBase.NestedReference].[Discriminator] IN (N'NestedReferenceDerived', N'NestedReferenceBase')
...
SELECT [e.Principals].[Id], [e.Principals].[Name], [e.Principals].[ReferenceId], [e.Principals].[ReferencedEntityId], [e.Principals.Reference].[Id], [e.Principals.Reference].[Name]
FROM [PrincipalEntity] AS [e.Principals]
LEFT JOIN [ReferencedEntity] AS [e.Principals.Reference] ON [e.Principals].[ReferenceId] = [e.Principals.Reference].[Id]
INNER JOIN (
    SELECT [e0].[Id]
